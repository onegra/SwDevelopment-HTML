...
<form method="POST" action="reply-write.jsp">
	...
	<textarea name="content" rows="5" cols="80" 
			...
			style="resize: none; height: 100px; padding: 10px; box-sizing: border-box;" ></textarea>
...
<div id="reply">
	...
	margin: 20px; padding: 10px; box-sizing: border-box;">
		<div style="display:flex; justify-content: space-between;">
		<div>${reply.writer }</div>
		<div><fmt:formatDate value="${reply.writeDate }" pattern="yyyy-MM-dd a hh:mm"></fmt:formatDate></div>
		<c:if test="${login.userid == reply.writer }">
		<div><a href="reply-delete.jsp?idx=${reply.idx }&board_idx=${param.idx }"><button>삭제</button></a></div>
		</c:if>
		</div>
