...
<%!
	...
	//선언부에서는 request 및 response에 접근 불가
%>
<%
	...
	String input = request.getParameter("user"); //입력값을 매번 새로 바뀐다
	int user; //사용자의 입력값은 매번 변경되기도 하고, request로 받아와야 한다 
	String msg ="";
	...
	if(input != null) {
		...
		if (user == cpu) {
			...
			msg = String.format("up, (%d회)",count);
		}
		else { //if(user > cpu)
			msg = String.format("down, (%d회)",count);
		}
	}
%>
<form>
