...
<%@page import="javax.swing.text.DefaultFormatterFactory"%>
...
<hr>
<%
	// 사용자가 파일을 첨부하여 제출하는 상황이라면 첨부파일의 크기를 미리 예측할 수 없다
	// 큰 용량의 파일을 단일 객체로 전송할 수 없기 때문에, 여러 파트로 분할하여 전송한 후 재조립한다
	//form-data를 여러 파트로 분할하여 전송하는 방식을 mutipart/form-data라고 한다
	
	//양식 데이터 전송 시 분할하여 제출하고,재조립하는 방식을 사용하기 때문에
	//일반적인 request 형식으로는 파라미터 및 파일을 확인할 수 없다
	//따라서 특정 라이브러리를추가로 사용해야 한다
	//com.oreilly.servlet.MutipartRequest
...
<%	
	//new MultipartRequest(request, 저장폴더, 최대 전송 크기, 글자 인코딩, 중복처리 정책);
